...
export default function ArtistCard({ artist, initialFollowing = false }: ArtistCardProps) {
  ...
  return (
    <div
      className="group relative flex flex-col items-center rounded-2xl bg-zinc-900/60 border border-zinc-800/60 hover:border-zinc-700 hover:bg-zinc-800/60 transition-all duration-200 cursor-pointer p-5 text-center"
      onClick={() => router.push(`/artist/${artist._id}`)}
    >
      {/* Avatar */}
      <div className="relative w-20 h-20 rounded-full overflow-hidden bg-zinc-800 border-2 border-zinc-700 group-hover:border-red-600/50 transition-colors mb-3">
        {avatarUrl ? (
          <img
            src={avatarUrl}
            alt={artist.name}
      ...
      {token && (
        <button
          ...
          className={`mt-3 flex items-center gap-1.5 px-4 py-1.5 rounded-full text-xs font-semibold transition-all
            ${following
